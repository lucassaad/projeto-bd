...
@router.get('/', response_model=Nurse_ubsOut, status_code=HTTPStatus.OK)
def get_nurse_ubs(nurse_cpf: str, ubs_cnes: str, db_session: db_session):
    nurse_ubs = select_nurse_ubs(nurse_cpf, ubs_cnes, db_session)
    if nurse_ubs is None:
...
def get_all_nurse_ubs(db_session: db_session):
    return select_all_nurse_ubss(db_session)
