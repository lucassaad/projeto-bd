...
@router.post('/', response_model=Patient_ubsOut, status_code=HTTPStatus.CREATED)
...
def get_patient_ubs_by_cnes(ubs_cnes: str, session: db_session):
    ...
    if patient_ubs is None:
        raise HTTPException(status_code=404, detail='No Patient UBS associations found for this CNES')
    return patient_ubs

@router.get('/all', response_model=list[Patient_ubsOut], status_code=HTTPStatus.OK)
...
def put_patient_ubs(id: int, patient_ubs_update: Patient_ubsUpdate, session: db_session):
    patient_ubs = update_patient_ubs(patient_ubs_update, id, session)
    if patient_ubs is None:
        raise HTTPException(status_code=404, detail='Patient UBS association not found')
    return patient_ubs

@router.delete('{id}', response_model=Patient_ubsOut, status_code=HTTPStatus.OK)
