...
from app.schemas.medication_prescription import Medication_preBase, Medication_preIn,Medication_preOut

router = APIRouter(prefix='/medication-pre', tags=['Medication_Prescription'])

db_session = Annotated[Session, Depends(get_session)]

@router.post('/', response_model=Medication_preOut, status_code=HTTPStatus.CREATED)
def post_medication_prescription(med_in: Medication_preIn, session: db_session):
...
@router.get('/pre', response_model=Medication_preOut, status_code=HTTPStatus.OK)
...
@router.get('/all', response_model=list[Medication_preOut], status_code=HTTPStatus.OK)
def get_all_medication_prescriptions(db_session: db_session):
    return select_all_medication_prescriptions(db_session) 



@router.put('/{id}', response_model=Medication_preOut, status_code=HTTPStatus.OK)
def put_medication_prescription(id: int, med_update: Medication_preIn, db_session: db_session):
...
@router.delete('/{id}', response_model=Medication_preOut, status_code=HTTPStatus.OK)
