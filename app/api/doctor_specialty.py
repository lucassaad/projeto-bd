...
@router.post(
    '/', response_model=DoctorSpecialtyOut, status_code=HTTPStatus.CREATED
)
def post_doctor_specialty(doc_in: DoctorSpecialtyIn, session: db_session):
    ...
    if user is None:
        raise HTTPException(
            ...
            detail='Doctor and specialty already exists',
        )
    return user


@router.get(
    '/specialty', response_model=DoctorSpecialtyOut, status_code=HTTPStatus.OK
)
def get_doctor_by_specialty(code: int, db_session: db_session):
    ...
    if user is None:
        raise HTTPException(
            status_code=404, detail='No doctor found for this specialty'
        )
...
@router.get(
    '/doctor', response_model=DoctorSpecialtyOut, status_code=HTTPStatus.OK
)
def get_specialty_by_doctor(cpf: str, db_session: db_session):
    ...
    if user is None:
        raise HTTPException(
            status_code=404, detail='No specialty found for this doctor'
        )
...
@router.get(
    '/all', response_model=list[DoctorSpecialtyOut], status_code=HTTPStatus.OK
)
...
@router.put(
    '/{id}', response_model=DoctorSpecialtyOut, status_code=HTTPStatus.OK
)
def put_user(id: int, user_update: DoctorSpecialtyIn, db_session: db_session):
    ...
    if user is None:
        raise HTTPException(
            status_code=404, detail='Doctor and specialty not found'
        )
