...
from app.schemas.doctor_ubs import Doctor_ubsIn, Doctor_ubsOut
...
def get_ubs_by_doctor(cpf: str, db_session: db_session):
    ...
    if user is None:
        raise HTTPException(
            status_code=404, detail='No ubs found for this doctor'
        )
...
@router.get(
    '/all', response_model=list[Doctor_ubsOut], status_code=HTTPStatus.OK
)
...
@router.delete(
    '/{id}', response_model=Doctor_ubsOut, status_code=HTTPStatus.OK
)
