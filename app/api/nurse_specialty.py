...
@router.post(
    '/', response_model=Nurse_specialtyOut, status_code=HTTPStatus.CREATED
)
def post_nurse_specialty(
    nurse_specialty_in: Nurse_specialtyIn, session: db_session
):
...
def get_nurse_specialty(nurse_cpf: str, session: db_session):
    ...
    if nurse_specialty is None:
        raise HTTPException(
            status_code=404, detail='Nurse_specialty not found'
        )
    return nurse_specialty
...
def put_nurse_specialty(
    id: int, nurse_specialty_update: Nurse_specialtyUpdate, session: db_session
):
    nurse_specialty = update_nurse_specialty(
        nurse_specialty_update, id, session
    )
    if nurse_specialty is None:
        raise HTTPException(
            status_code=404, detail='Nurse_specialty not found'
